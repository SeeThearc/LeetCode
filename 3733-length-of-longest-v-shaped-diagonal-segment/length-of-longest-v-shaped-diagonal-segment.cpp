...
public:
    ...
    int lenOfVDiagonal(vector<vector<int>>& grid) {
        ...
        for(int i=0;i<m;i++){
            for(int j=0;j<n;j++){
                if(grid[i][j]==1){
                    for(int d=0;d<=3;d++){
                        result = max(result,1 + solve(i,j,d,true,2,grid,m,n));
                    }
                }
            }
        }
    ...
    int solve(int i,int j,int d,bool canTurn,int val,vector<vector<int>>&grid,int m,int n){
        ...
        if(canTurn){
            int turnAndFind = max(keepMoving,1+solve(i_,j_,(d+1)%4,false,val==2?0:2,grid,m,n));
